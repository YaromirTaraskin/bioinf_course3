...
def process_fastq_perform_analyses(filename):
    """Main function to process the FASTQ file and perform analyses.

    Args:
        filename (str | os.PathLike): The path to the FASTQ file.
    """
    try:
        # Open and parse the FASTQ file
        print()
        print(f'Processing "{filename}" ')
        print("=" * len(f'Processing "{filename}"'))
        with gzip.open(filename, "rt", encoding="utf-8") as f:
            ...
            plot_n_calls(counts_n_per_pos, f"n_calls.{filename}.png")
            analyze_quality_scores(quality_counts)
            plot_quality_distributions(quality_per_pos, f"phred.{filename}.png")
...
def plot_quality_distributions(
    ...
    ax.set_xticklabels([str(pos) for pos in positions], rotation=90)
    ax.set_xlabel("Read distance", fontsize="xx-large")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        ...
        print("#")
        arg_filename = "SRR003265.filt.fastq.gz"
        print(f'Using "{arg_filename}" as example')
    ...
    process_fastq_perform_analyses(arg_filename)
